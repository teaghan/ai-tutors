import streamlit as st
from utils.tutor_data import select_instructions, available_tutors
from utils.chatbot_setup import reset_chatboat 

def load_tool(df_tutors, tool_name):
    (st.session_state["instructions"], 
    st.session_state["guidelines"], 
    st.session_state["introduction"]) = select_instructions(df_tutors, 
                                                           tool_name=tool_name)
    st.session_state["tool_name"] = tool_name
    st.switch_page('pages/tutor.py')

def load_editor(df_tutors, tool_name):
    (st.session_state["instructions_cur"], 
    st.session_state["guidelines_cur"], 
    st.session_state["introduction_cur"]) = select_instructions(df_tutors, 
                                                           tool_name=tool_name)
    st.session_state["tool_name_cur"] = tool_name
    st.switch_page('pages/create_tutor.py')

def display_tools(show_all=True, allow_edit=False):
    if show_all:
        suffix = 'pub'
    else:
        suffix = 'usr'
    df_tutors = st.session_state["df_tutors"]
    avail_tools = available_tutors(df_tutors)
    for name, desc, creator_email in avail_tools:
        # Check if the creator email matches the current user's email
        if creator_email==st.session_state["user_email"] or show_all:
            with st.container():
                st.subheader(name)  # Display tool name
                st.write(desc)      # Display tool description
                # Button to load the selected tool
                if st.button(f"Interact", key=f'{name}_inter_{suffix}'):
                    reset_chatboat()
                    load_tool(df_tutors, name)
                if allow_edit:
                    if st.button(f"Edit", key=f'{name}_edit_{suffix}'):
                        reset_chatboat()
                        load_editor(df_tutors, name)
